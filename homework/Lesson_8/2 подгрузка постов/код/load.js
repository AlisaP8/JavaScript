...
// 1. Получаем ссылку на .scrollCheck
 ...
 */
function init() {
    // 4. Если условие истинно, то вызываем insertPosts и рекурсивно вызываем init.
...
document.addEventListener('scroll', function (event) {
    if (scrollCheck.getBoundingClientRect().top <= window.innerHeight) {
        // 5. Если условие истинно вызываем insertPosts.
        insertPosts();
    }
});
...
function insertPosts() {
    // 3. в postsMarkup дописываем с помощью конкатенации
    // разметку, возвращаемую getPostMarkup.
    // 3.1 в getPostMarkup в качестве аргумента передаем счетчик цикла
    let postsMarkup = '';
    for (let i = 0; i < 3; i++) {
        postsMarkup += getPostMarkup(i);
    }
    scrollCheck.insertAdjacentHTML('beforebegin', postsMarkup);
    // 3.2 Перед scrollCheck вставляем записанную в postsMarkup разметку
...
function getPostMarkup(counter) {
    /* 2.В num записывается текущее время unix плюс counter
    */
